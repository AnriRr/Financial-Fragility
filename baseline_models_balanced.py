...
OUT_DIR = "baseline_results_balanced"
...
def objective_xgb(trial, X_train, y_train, cv_inner, base_pos_weight):
...
for train_idx, test_idx in outer_cv.split(X, y):
    ...
    logit_params["class_weight"] = "balanced"
    ...
    outer_conf_matrices["Logistic Regression"].append(confusion_matrix(y_te, y_pred))
    ...
    outer_conf_matrices["Random Forest"].append(confusion_matrix(y_te, y_pred))
    ...
    study_xgb.optimize(
        lambda trial: objective_xgb(trial, X_tr_scaled, y_tr, cv_inner, base_pos_weight),
        n_trials=30,
        show_progress_bar=False
    )
    ...
    outer_conf_matrices["XGBoost"].append(confusion_matrix(y_te, y_pred))
...
results_df.to_csv(f"{OUT_DIR}/baseline_results_balanced.csv")
